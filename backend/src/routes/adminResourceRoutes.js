...
function parseResourceRoute(pathname, basePath) {
...
export async function handleAdminResourceRoutes({
  request,
  response,
  pathname,
  searchParams,
  ...
  const route = parseResourceRoute(pathname, basePath);
  ...
  try {
    ...
    if (request.method === "POST" && route.id === undefined) {
      sendStoreResult(response, store.create(await readJsonBody(request)));
      return true;
    }

    if (request.method === "PUT" && route.id !== undefined && route.action === undefined) {
      sendStoreResult(response, store.update(route.id, await readJsonBody(request)));
      return true;
    }

    if (request.method === "PATCH" && route.id !== undefined && route.action === "status") {
      const body = await readJsonBody(request);
      sendStoreResult(response, store.setStatus(route.id, body.status));
      return true;
    }
